...
def main():
    ...
    if 'processing_done' in st.session_state and st.session_state['processing_done']:
        ...
        if selected_content:
            ...
            # 3. Редагування контенту в поточному порядку
            st.markdown("### ✏️ Редагування вмісту")
            for i, block in enumerate(ordered_content):
                unique_key = f"edit_{block['filename']}_{block['idx']}"
                
                # Ініціалізуємо початковим текстом, якщо ще немає в сесії
                if unique_key not in st.session_state['edited_texts']:
                    st.session_state['edited_texts'][unique_key] = block['content']
                
                label = f"Блок {i+1}: {block['header']}" if block.get('header') else f"Блок {i+1} (без заголовка)"
                
                edited_text = st.text_area(
                    label,
                    value=st.session_state['edited_texts'][unique_key],
                    key=f"ta_{unique_key}",
                    height=150
                )
                
                # Зберігаємо зміни
                st.session_state['edited_texts'][unique_key] = edited_text
                # Оновлюємо об'єкт для експорту
                block['content'] = edited_text
        else:
            ordered_content = []
